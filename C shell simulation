...
/*
typedef: a nickname of another data
*/

//creating a new struct and call it CMD
//what does this datatype mean
typedef struct parse_cmd{
	int background;   //if the process is running in the background
	int type;         // how to deal with this cmd
	char* file;
	char** args2;
}CMD;

void read_cmd(char* cmd);//read the input
char* skip_space(char* cmd);//skip the space character
...
int main()
{
	char cmd[CMD_L];//command
	char dir[CMD_L];//direction
	char* args[CMD_L];
	CMD info;
	while(1)//contineous receiving input
	{
		getcwd(dir,CMD_L);        // get the directory
		printf("[23_Shell]: ~%s >> ""\x1b[0m",dir);
			//printf("Enter read_cmd()\n");
		read_cmd(cmd);//read command
			//printf("Exit read_cmd()\n");
		if(cmd == NULL) continue;
		if(strcmp(cmd,"exit") == 0) break;//entering == exit => end process 
			//printf("Enter split()\n");
		split(args,cmd);
			//printf("Exit split()\n");
			//printf("Enter parse()\n");
...
{
	memset(cmd,'\0',sizeof(char)*CMD_L);//change cmd to a list of end-of-string
	
	if(!fgets(cmd,CMD_L,stdin))//didn't receive command
	{
		cmd = NULL;
		return;
	}
		
	skip_space(cmd);//skip the space to get next part of command
	if(strlen(cmd) == 0)
	{
		cmd = NULL;
		return;
...
char* skip_space(char* cmd)
{
	if(!cmd) 
		return cmd;
	while(isspace(*cmd))//if not a space
		++cmd;//move to next word
		
	char* tail = cmd;
	while(*tail != '\0')//still not the tail, move to next
		++tail;//fount the character behind the tail,move back once 
	--tail;
...
{
	int i = 0;
	memset(args,'\0',sizeof(char *)*CMD_L);//allocate a string array which it's length is command_L
...
CMD parse(char* args[])
{
	...
	int count = 0;
	int error = 0;
	while(args[count])//args[count] has something, count until command end
		++count;
	--count;
	...
	for(int i = 0 ; i <= count ; ++i)
	{
		...
		else if(strcmp(args[i],"|") == 0)//take first output as second process's input
		{
